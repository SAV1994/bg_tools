...
class Gamers extends Table {
  ...
  TextColumn get firstName => text().withLength(min: 1, max: 20)(); // Имя
  TextColumn get lastName =>
      text().withLength(min: 1, max: 20).nullable()(); // Фамилия
  TextColumn get middleName =>
      text().withLength(min: 1, max: 20).nullable()(); // Отчество
  BoolColumn get isOwner =>
      boolean().withDefault(const Constant(false))(); // Владелец?
