import 'package:bg_tools/core/providers/paginated_providers/base.dart';
import 'package:bg_tools/features/top/consts.dart';
import 'package:flutter_riverpod/flutter_riverpod.dart';

import 'package:bg_tools/core/providers/database_providers.dart';

final ratingsPaginatedProvider =
    AsyncNotifierProvider<RatingsNotifier, List<dynamic>>(() {
      return RatingsNotifier();
    });

class RatingsNotifier extends BaseNotifier {
  @override
  bool reverseOrdering = true;
  TopTypeEnum ratingType = TopTypeEnum.common;
  int? instanceId;

  @override
  Future<List<dynamic>> load() async {
    final dao = ref.read(ratingDaoProvider);

    totalItems = await dao.getTotalCount(
      ratingType: ratingType,
      instanceId: instanceId,
    );

    return await dao.getPaginated(
      page: page,
      pageSize: pageSize,
      reverseOrdering: reverseOrdering,
      ratingType: ratingType,
      instanceId: instanceId,
    );
  }

  Future<void> filterByInstanceId(int? id) async {
    instanceId = id;
    page = 0;
    state = await AsyncValue.guard(() => load());
  }

  Future<void> setTopType(TopTypeEnum topType) async {
    ratingType = topType;
    page = 0;
    state = await AsyncValue.guard(() => load());
  }

  @override
  void resetAdditionalParams() async {
    ratingType = TopTypeEnum.common;
    instanceId = null;
    state = await AsyncValue.guard(() => load());
  }
}
